...
class Day2 {
    ...
    data class Line(val min: Int, val max: Int, val char: Char, val pass: String)

    @Test
    fun part1() {
        val valid = input.lines()
                .map {
                    val match = Regex("(\\d+)-(\\d+) (\\w): (\\w+)").matchEntire(it)!!
                    Line(
                            min = match.groupValues[1].toInt(),
                            max = match.groupValues[2].toInt(),
                            char = match.groupValues[3].first(),
                            pass = match.groupValues[4]
                    )
                }
                .filter { line ->
                    val count = line.pass.count { char -> char == line.char }
                    val valid = line.min <= count && count <= line.max
                    valid
                }
                .count()
        println(valid)
    ...
    fun part2() {
        val valid = input.lines()
                ...
                .filter { line ->
                    val first = line.pass[line.min - 1] == line.char
                    val second = line.pass[line.max - 1] == line.char
